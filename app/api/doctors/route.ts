...
import { createDoctorSchema, listDoctorsQuerySchema } from '@/lib/zod-schemas/doctor';
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { success: false, error: 'INTERNAL_ERROR', message: 'Failed to list doctors' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error instanceof Error && error.message.includes('Unique constraint')) {
      return NextResponse.json(
        { success: false, error: 'DUPLICATE', message: 'Phone number or registration already exists' },
        { status: 409 }
      );
    }

    return NextResponse.json(
      { success: false, error: 'INTERNAL_ERROR', message: 'Failed to create doctor' },
      { status: 500 }
    );
  }
}
